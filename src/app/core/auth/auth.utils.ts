...
export class AuthUtils
{
    ...
    /**
     * Vérifie si le token d'accès JWT est expiré.
     *
     * @param accessToken Le token JWT à vérifier.
     * @returns true si le token est expiré, false autrement.
     */
    public static isAccessTokenExpired(accessToken: string): boolean {
        // Décode le token pour obtenir les claims
        const decodedToken = this._decodeToken(accessToken);

        // Vérifie l'existence du token et de la propriété exp
        if (!decodedToken || !decodedToken.exp) {
            return true; // Considère le token comme expiré si non décodable ou sans exp
        }

        // Obtient la date d'expiration du token
        const expirationDate = this._getTokenExpirationDate(accessToken);

        // Si la date d'expiration ne peut pas être déterminée, considère le token comme expiré
        if (!expirationDate) {
            return true;
        }
